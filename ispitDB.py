...
class IspitiDB():
    ...
    def promijeni_student(self, current_ime_prezime, new_ime_prezime):
        ...
        if stud_id:
            self.cur.execute("UPDATE studenti SET ime_prezime = (?) WHERE student_id LIKE (?)",
                             (new_ime_prezime, stud_id))
            self.conn.commit()
            return self.cur.lastrowid
        else:
            return None
    ...
    def ispitaj(self, student, kolegiji, ocjena=None):
        self.cur.execute("SELECT 1 FROM ispiti WHERE student_id = (?) AND kolegiji_id = (?)",
                         (self.vrati_student_id(student), self.vrati_kolegiji_id(kolegiji)))
        row = self.cur.fetchone()

        if ocjena == None:
            if row:
                self.cur.execute("DELETE FROM ispiti WHERE student_id = (?) AND kolegiji_id = (?)",
                                 (self.vrati_student_id(student), self.vrati_kolegiji_id(kolegiji)))
                self.conn.commit()
        else:
            if row:
                self.cur.execute("UPDATE ispiti SET ocjena = (?) WHERE student_id = (?) AND kolegiji_id = (?)",
                                 (ocjena, self.vrati_student_id(student), self.vrati_kolegiji_id(kolegiji)))
                self.conn.commit()
            else:
                self.cur.execute("INSERT INTO ispiti (student_id, kolegiji_id, ocjena) VALUES (?, ?, ?)",
                                 (self.vrati_student_id(student), self.vrati_kolegiji_id(kolegiji), ocjena))
                self.conn.commit()
